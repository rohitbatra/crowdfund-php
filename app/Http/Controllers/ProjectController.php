...
class ProjectController extends Controller {
	...
	public function projectsSearch(Request $request){
		$this->layout = 'project.list';
		$this->metas['title'] = 'Projects | "'.$request->get('searchtext').'" Search results ';
		// General list
	...
	public function add(){
		...
		$contentData = [
			['id'=>'faq','title'=>'FAQ','content'=>Content::getContent('faq')],
			['id'=>'requirement','title'=>'Requirement','content'=>Content::getContent('requirment')],
			['id'=>'funding','title'=>'Funding','content'=>Content::getContent('funding')],
			['id'=>'tos','title'=>'Terms of Service','content'=>Content::getContent('tos')],
		];
	...
	public function project($slug){
		...
		if($project->exists()){
			...
			$this->metas['title'] = 'Project Not Found';
			$this->layout = 'errors.404';
			$status = "Your Project was not found or maybe inactive. Contact us at support@poloniago.com";
			$project = null;
		}
		$this->view = $this->BuildLayout();
		return $this->view
	...
	public function addGoalModal(){
		$addGoalModal = view('modules.modal', ['id'=>'addgoalmodal','title' => 'Add Project Goals','modalbody'=>'modules.project.goal_add'])
			->render()
		;
		$return['status'] = true;
		...
		if ($v->fails()){
			...
			if($request->has('project_id')){
				...
				if($project && $project->user_id == $this->user->id && $project->status == 0){
					...
					$return['errors'] = ['This is not your Project.'];
				}
			} else {
				$return['status'] = false;
	...
	public function addRewardModal(){
		$addRewardModal = view('modules.modal', ['id'=>'addrewardmodal','title' => 'Add Project Reward','modalbody'=>'modules.project.reward_add'])
			->render()
		;
		$return['status'] = true;
	...
	public function claimRewardModal(Request $request){
		...
		$rewardid = $request->get('rewardid');
		$reward = Reward::find($rewardid);
		$claimRewardModal = view('modules.modal', ['id'=>'claimrewardmodal'.$rewardid,'title' => 'Claim Reward Modal','modalbody'=>'modules.project.reward_claim'])
			->withUser($this->user)
			->withCol($col)
			->withReward($reward)
			->render()
	...
	public function claimReward(){
		$addRewardModal = view('modules.modal', ['id'=>'addrewardmodal','title' => 'Claim Reward','modalbody'=>'modules.project.reward_add'])
			->render()
		;
		$return['status'] = true;
		$return['view'] = $addRewardModal;
	...
	public function supporterListModal(Request $request){
		...
		if($this->user->id == $project->user_id || $this->user->role == 1){
			$supporterListModal = view('modules.modal', ['id'=>'supporterlistmodal'.$project->id,'title' => 'Your '.$project->title.' Project supporters ','modalbody'=>'modules.project.supporter_list','size'=>'lg'])
				->withProject($project)
				->render()
				;
		}
